...
class TestDwcArchivePackageTask(object):
    ...
    def _register_uri(self):
        ...
        httpretty.register_uri(
            ...
            responses=[
                # the first request will be just for the fields
                httpretty.Response(json.dumps(data)),
                # then a normal data request
                httpretty.Response(json.dumps(data)),
                # then we indicate that there's no more data
                httpretty.Response(json.dumps({'result': {'records': []}}))
            ]
        )
    ...
    def test_expected_extensions(self):
        ...
        assert_equals(set(r.csv_files.keys()),
                      {'occurrence.csv', 'measurement_or_fact.csv', 'multimedia.csv'})
    ...
    def test_unknown_fields_in_dynamic_properties(self):
        """
        Ensure that unknown fields are mapped to dynamic properties, and that the content of the row
        contains the expected value pairs
        """
        self._register_uri()
        r = DummyResource()
        self._task.create_zip(r)
        ...
        expected = {"lesserknownfield": "row 1 lesser known field",
                    "unknownfield": "row 1 unknown field"}
        assert_equals(r.csv_files['occurrence.csv'][1][4], json.dumps(expected))
